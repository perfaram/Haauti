...
class JodelAccount: NSDocument, CLLocationManagerDelegate {
    ...
    public init(type typeName: String) throws {
        ...
        if CLLocationManager.authorizationStatus() == CLAuthorizationStatus.authorized,
            ...
            //todo : ask user, don't just pin to Berlin
            
        }
        
        self.country = "DE"
        promisedCity = self.city(atLocation: self.location)
        let rn = self.renewAllTokens()
        rn.then { (bag) in
    ...
    override func read(from data: Data, ofType typeName: String) throws {
        ...
        if let account_config = dict["account_config"] as? NSDictionary {
            
        }
        if let app_settings = dict["associated_settings"] as? NSDictionary {
            if let settings = AssociatedSettings.fromCocoaDict(app_settings) {
                self.settings = settings
            }
        }
        if let feed_types = dict["feed_types"] as? NSDictionary {
            
        }
        
        let aLocation = CLLocationCoordinate2D(latitude: lat, longitude: lng)
        if self.settings.followUserLocation == true {
    ...
    public var location: CLLocationCoordinate2D = CLLocationCoordinate2D(latitude: 52.52437, longitude: 13.41053) {
        didSet {
            guard (oldValue != location) else { return }
            
            guard self.authenticationBag != nil else { return }
            ...
            Promises.all([cityPromise]).then({ values -> Promise<JSON> in
                let city = values[0]
                ...
                return self.jodelRequest(.put, "/v2/users/location", params: nil, payload: data, authenticated: true)
    ...
    public var settings : AssociatedSettings = AssociatedSettings()
    ...
    fileprivate var accountDelegates = [JodelAccountDelegate]()
    ...
    public func register(accountDelegate delegate: JodelAccountDelegate) {
        accountDelegates.append(delegate)
    }
    ...
    func toCocoaDict() -> NSDictionary {
        ...
        dict["associated_settings"] = self.settings.toCocoaDict()
        return dict
    }
    ...
    func setConfig() -> Promise<Bool> {
        let path = "/v3/user/config"
        return self.jodelRequest(.options, path, params: nil, payload: nil, authenticated: true)
            .then { (json) -> Promise<Bool> in
                Swift.print(json)
                return Promise(value: true)//todo real return val
            }.recover({ (error) -> Promise<Bool> in
    ...
    func getConfig() -> Promise<Bool> {
        let path = "/v3/user/config"
        return self.jodelRequest(.get, path, params: nil, payload: nil, authenticated: true)
            .then { (json) -> Promise<Bool> in
                Swift.print(json)
                return Promise(value: true)//todo real return val
            }.recover({ (error) -> Promise<Bool> in
    ...
    func getKarma() -> Promise<Int> {
        let path = "/v2/users/karma"
        return self.jodelRequest(.get, path, params: nil, payload: nil, authenticated: true)
            .then { (json) -> Promise<Int> in
                guard let karma = json["karma"].int
                    else { //todo better value checking
                        return Promise(error: JodelError.APIError(.MalformedResponse))
                }
                self.accountDelegates.forEach({ (delegate) in
                    delegate.update(karma: karma)
                })
                return Promise(value: karma)
            }
    }
