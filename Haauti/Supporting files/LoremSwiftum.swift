...
public final class Lorem {
    ...
    public static let fakery = Faker.init()
    ...
    public static var randomHashtag: String {
        return fakery.internet.hashtag()
    }
    
    /// Generates a random jodel which is shorter than 255 characters.
    public static func jodel(length: Int = 255) -> String {
        var tweet = ""
        
        while tweet.count < length {
            tweet += sentence
            if (Int.random(min: 0, max: 2) <= 1) {
                tweet += randomEmoji + " "
            }
            if (Int.random(min: 0, max: 8) < 10*tweet.count/255) {
                break
            }
        }
        
        if tweet.count < length {
            if (Int.random(min: 0, max: 5) < 3) {
                tweet += randomHashtag + " "
                if (Int.random(min: 0, max: 7) < 3) && (tweet.count < length) {
                    tweet += randomHashtag + " "
                }
            }
        }
        
        tweet = String(tweet.prefix(length - 1))
        tweet = tweet.trimmingCharacters(in: CharacterSet.whitespacesAndNewlines)
...
extension Date {
    func generateRandomDate(daysBack: Int) -> Date {
        ...
        let randomDate = gregorian.date(byAdding: offsetComponents, to: today, wrappingComponents: false)
